...
def test_multihead_cnn_forward():
    ...
    model = SimpleCNN(
        input_channels=1, heads_config={"digit": 10, "thickness": 5, "smoothness": 3}
    )
...
def test_multihead_dataset():
    ...
    for digit, expected_thickness, expected_smoothness in test_cases:
        ...
        assert (
            labels["thickness"] == expected_thickness
        ), f"Digit {digit}: thickness {labels['thickness']} != {expected_thickness}"
        assert (
            labels["smoothness"] == expected_smoothness
        ), f"Digit {digit}: smoothness {labels['smoothness']} != {expected_smoothness}"
...
def test_lightning_module_backward_compatibility():
    ...
    module = MNISTLitModule(
        net=net, optimizer=torch.optim.Adam, scheduler=None, criterion=criterion
    )
...
def test_digit_mappings_exhaustive():
    ...
    for digit, expected in expected_mappings.items():
        ...
        ), f"Digit {digit}: thickness {thickness} != {expected['thickness']}"
        assert (
            smoothness == expected["smoothness"]
        ), f"Digit {digit}: smoothness {smoothness} != {expected['smoothness']}"
